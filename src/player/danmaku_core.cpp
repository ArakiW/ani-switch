...
// Adapted from wiliwili; HTTP mask transport uses the application client.
...
static void fetchMaskRange(const std::string& url, uint64_t start, uint64_t end,
    std::function<void(const std::string&)> callback, ErrorCallback error) {
    ...
    headers["Range"] = "bytes=" + std::to_string(start) + "-" +
        (end == std::numeric_limits<size_t>::max() ? "" : std::to_string(end));
    session->SetHeader(headers);
    if (!hostForHeader.empty()) {
        session->UpdateHeader(cpr::Header{{"Host", hostForHeader}});
    }
    session->GetCallback([callback, error](const cpr::Response& response) {
        brls::sync([callback, error, response] {
            if (response.error || response.status_code != 206)
                fireError(error, "Mask range request failed", static_cast<int>(response.status_code));
            else if (callback) callback(response.text);
        });
    });
}

static std::string decompressGzipData(const std::string& input) {
    z_stream stream{};
    if (inflateInit2(&stream, 15 + 32) != Z_OK) throw std::runtime_error("Cannot initialize gzip");
    ...
    try {
        do {
            ...
            if (status != Z_OK && status != Z_STREAM_END) throw std::runtime_error("Invalid gzip data");
            result.append(buffer.data(), buffer.size() - stream.avail_out);
            if (result.size() > 64 * 1024 * 1024) throw std::runtime_error("Mask exceeds memory limit");
...
static void base64Decode(const std::string& input, std::string& output) {
    ...
    int status = mbedtls_base64_decode(nullptr, 0, &size, bytes, input.size());
    if (status != 0 && status != MBEDTLS_ERR_BASE64_BUFFER_TOO_SMALL)
        throw std::runtime_error("Invalid base64 data");
    output.resize(size);
    if (mbedtls_base64_decode(reinterpret_cast<unsigned char*>(output.data()), size, &size, bytes, input.size()) != 0)
        throw std::runtime_error("Invalid base64 data");
    output.resize(size);
}
...
DanmakuItem::DanmakuItem(std::string content, const char *attributes) : msg(std::move(content)) {
    ...
    if (msg.size() > 1 && msg[0] == '[' && msg[msg.size() - 1] == ']') {
        ...
        } else if (pystring::startswith(pystring::upper(msg.substr(0, 4)), "[OHH")) {
...
DanmakuCore::DanmakuCore() {
    DANMAKU_FONT = brls::Application::getDefaultFont();
    this->reset();
...
void DanmakuCore::loadDanmakuData(const std::vector<DanmakuItem> &data) {
    ...
    // 通过mpv来通知弹幕加载完成
    applicationEvent.fire("DANMAKU_LOADED", nullptr);
}
    ...
    fetchMaskRange(
        ...
        [this, url](const std::string &text) {
            ...
            fetchMaskRange(
                ...
                [](const std::string& error, int) { brls::Logger::error("get web mask 2: {}", error); });
        },
        [](const std::string& error, int) { brls::Logger::error("get web mask 1: {}", error); });
}
...
void DanmakuCore::save() {
    ...
    ProgramConfig::instance().setSettingItem(SettingItem::DANMAKU_FILTER_SHOW_TOP, DANMAKU_FILTER_SHOW_TOP, false);
    ProgramConfig::instance().setSettingItem(SettingItem::DANMAKU_FILTER_SHOW_BOTTOM, DANMAKU_FILTER_SHOW_BOTTOM, false);
    ProgramConfig::instance().setSettingItem(SettingItem::DANMAKU_FILTER_SHOW_SCROLL, DANMAKU_FILTER_SHOW_SCROLL, false);
    ProgramConfig::instance().setSettingItem(SettingItem::DANMAKU_FILTER_SHOW_COLOR, DANMAKU_FILTER_SHOW_COLOR, false);
    ProgramConfig::instance().setSettingItem(SettingItem::DANMAKU_FILTER_SHOW_ADVANCED, DANMAKU_FILTER_SHOW_ADVANCED, false);
...
void WebMask::parseHeader2(const std::string &text) {
    ...
    for (int32_t i = 0; i < length; i++) {
        ...
        if (i == length - 1) sliceList[i].offsetEnd = SIZE_MAX;
        currentOffset += 16;
    }
    ...
    fetchMaskRange(
        ...
        [this, slices, requestStart, requestEnd](const std::string &text) {
            ...
            for (size_t i = requestStart; i < requestEnd; i++) {
                ...
                std::string data;
                try {
                    if (slice.offsetEnd == SIZE_MAX) slice.offsetEnd = text.size() + offset;
                    data = decompressGzipData(
