...
#include <utility>
#include "player/recycling_grid.hpp"
#include "ui/view/button_refresh.hpp"
...
RecyclingGrid::RecyclingGrid() {
    ...
    // 使用 Box �?addView，添加一�?detached button
...
void RecyclingGrid::draw(NVGcontext* vg, float x, float y, float width, float height, brls::Style style,
    ...
    // todo：只在滑动过程中调用 itemsRecyclingLoop 以节省静止时的计算消�?
    itemsRecyclingLoop();
...
void RecyclingGrid::addCellAt(size_t index, bool downSide) {
    ...
    if (isFlowMode) {
        // 必须�?getHeight 前设置宽度，否则会影响到cell自定义高度的判定
        cell->setWidth(cellWidth);
        if (cellHeightCache[index] == -1) {
            // 没有预定义cell的高度，使用cell默认的高�?
...
void RecyclingGrid::reloadData() {
    ...
    if (renderedFrame.size.width != renderedFrame.size.width) {
        // 当列表在展示骨架屏后被隐藏，这时获取�?width 的值为 NAN
        // 使用历史宽度值避免后续计算错�?
    ...
    if (!isFlowMode || spanCount != 1) {
        ...
        // 这里添加首项是因为添加首项时会变�?renderedFrame �?height 值，包括 itemsRecyclingLoop 内的计算也都是以首项为基准进行的
        // 原则上这里的 addCellAt 任意添加一项即可（比如添加第零项），但最好能添加�?cellFocusIndex 附近，这有助于提升首屏性能
        size_t lineHeadIndex = cellFocusIndex / spanCount * spanCount;
        // 更新 renderedFrame 数据，设置y的值，伪装成已经移除了 lineHeadIndex 项之前的列表�?
        // y 值表示当前列表渲染的顶部，低�?y 值高度的列表项不会被渲染
        renderedFrame.origin.y = getHeightByCellIndex(lineHeadIndex);

        // 添加 lineHeadIndex 项到需要渲染的列表项中，因�?lineHeadIndex 为一行的首项，在执行 addCellAt 之后会更�?renderedFrame 数据
        // renderedFrame �?height 调整为第 lineHeadIndex 项的高度
        ...
        // 流式布局无法准确确定焦点cell的位置，因此暂时只添加第一项，�?itemsRecyclingLoop 中会逐渐添加�?cellFocusIndex，再按需删除
        // �?cellFocusIndex 过于大时，会导致添加的元素过多，因此需要考虑优化
        this->addCellAt(0, true);
    }

    // 在前面的操作中，列表增加了一项，通过 selectRowAt 再精确地显示出具体选中�?
    selectRowAt(cellFocusIndex, false);
}

void RecyclingGrid::notifyDataChanged() {
    // todo: 目前仅能处理data在原本的基础上增加的情况，需要考虑data减少或更换时的情�?
    if (!layouted) return;
...
void RecyclingGrid::itemsRecyclingLoop() {
    ...
    while (true) {
        ...
        // 当第一个cell的顶�?�?组件顶部的距离大�?preFetchLine 行元素的距离时结�?
    ...
    while (true) {
        ...
        // 当最后一个cell的顶�?�?组件底部间的距离 小于 preFetchLine 行元素的距离时结�?
    ...
    while (visibleMin - 1 < dataSource->getItemCount()) {
        if ((visibleMin) % spanCount == 0)
            // �?renderedFrame 顶部 �?组件顶部的距离小�?preFetchLine 行cell的距离时结束
    ...
    while (visibleMax + 1 < dataSource->getItemCount()) {
        ...
        if ((visibleMax + 1) % spanCount == 0)
            // 如果 renderedFrame 底部 �?组件底部 距离超过了preFetchLine 行cell的距离时结束
    ...
    if (visibleMax + 1 >= this->getItemCount()) {
        // 只有�?requestNextPage 为false时，才可以请求下一页，避免多次重复请求
