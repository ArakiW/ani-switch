...
public:
    ...
    /// 瀑布流模式，每一项高度不固定（仅在spanCount�?时可用）
...
private:
    ...
    // true表示正在请求下一页，此时不会再次触发下一页请�?
    // 数据为空时不请求下一页，因为有些时候首页和下一页请求的内容或方式不�?
    ...
    /**
     * 在指定位置添加一个列表项
     * 内部更新 renderedFrame 的值，假设有一个每一项都绘制的超长列表，renderedFrame �?y 表示当前截取绘制的顶部坐标，height 表示当前绘制的高�?
     * 当添加一个列表项时，renderedFrame �?height 增加一项的高度（注意，只在每行的第一个列表项添加时才更新列表项的高度�?
     * @param index 指定的位�?
     * @param downSide 是向下添加还是向上添加，当向上添加时 �?renderedFrame �?y 减去当前列表项的高度。（y 的值只在向上添加或移除时候改变）
     */
