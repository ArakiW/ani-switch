...
// Adapted from wiliwili for ani-switch configuration and lifecycle.
...
#endif
static void playbackTrace(const char* message) {
#if defined(__SWITCH__) && defined(ANISWITCH_SWITCH_DEBUG)
...
void MPVCore::on_update(void *self) {
    brls::sync([]() {
        if (!MPVCore::instance().getContext()) return;
...
void MPVCore::on_wakeup(void *self) {
    brls::sync([]() { if (MPVCore::instance().getHandle()) MPVCore::instance().eventMainLoop(); });
}
...
void MPVCore::init() {
...
#endif
    brls::Logger::info("MPV Version: {}", getString("mpv-version"));
    brls::Logger::info("FFMPEG Version: {}", getString("ffmpeg-version"));
    command_async("set", "audio-client-name", "ani-switch");
    ...
    playbackTrace("player: mpv render context ready");
#if defined(__SWITCH__)
    playbackTrace("player: mpv init complete");
#endif
}
...
void MPVCore::initializeVideo() {
    ...
    playbackTrace("player: initializeVideo FB begin");
    this->mpv_fbo.w = (int)brls::Application::windowWidth;
    this->mpv_fbo.h = (int)brls::Application::windowHeight;
    ...
    playbackTrace("player: initializeVideo FB done");
#endif
}
    ...
    while (true) {
        ...
        switch (event->event_id) {
            ...
            case MPV_EVENT_PLAYBACK_RESTART:
                playbackTrace("player: playback started or seek completed");
            ...
            case MPV_EVENT_END_FILE: {
                ...
                if (node->reason == MPV_END_FILE_REASON_ERROR) {
                    playbackTrace("player: file error");
                ...
                switch (event->reply_userdata) {
                    ...
                    case 4:
                        ...
                        if (((mpv_event_property *)event->data)->data) {
                            ...
                            if (video_progress != (int64_t)playback_time) {
                                ...
                                mpvCoreEvent.fire(MpvEventEnum::UPDATE_PROGRESS);
                                // 判断是否需要暂停播放
                                if (CLOSE_TIME > 0 && static_cast<size_t>(std::time(nullptr)) > CLOSE_TIME) {
                                    CLOSE_TIME = 0;
                                    this->pause();
                                }
                            }
                    ...
                    case 10:
                        ...
                        if (data) {
                            ...
                            mpvCoreEvent.fire(MpvEventEnum::VIDEO_SPEED_CHANGE);
                        }
                        break;
                    case 11:
                        // 音量信息
                        if (data) {
                            if (*(int64_t *)data > 0 && volume == 0) {
                                mpvCoreEvent.fire(MpvEventEnum::VIDEO_UNMUTE);
                            } else if (*(int64_t *)data == 0 && volume > 0) {
                                mpvCoreEvent.fire(MpvEventEnum::VIDEO_MUTE);
                            }
                            volume = *(int64_t *)data;
...
void MPVCore::disableDimming(bool disable) {
    ...
    brls::Application::getPlatform()->disableScreenDimming(disable, "Playing video", "ani-switch");
