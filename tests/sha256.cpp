...
//
// SHA-256 (FIPS 180-2) reference implementation.  Not constant-time —
// only suitable for test fixtures, not production crypto.
...
void update(Context& ctx, const uint8_t* data, std::size_t len) {
    ...
    while (len > 0) {
        const std::size_t take = std::min<std::size_t>(64 - ctx.buflen, len);
